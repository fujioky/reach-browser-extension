...
| 弹窗：当前帖子、本次链接的访问控制、最近的分享 | 设置：管理员账号登录、默认分享设置 |
...
- **分享在后台跑，弹窗只负责显示。** 弹窗一失去焦点就会被浏览器关掉，没法承载要等几十秒的请求。弹窗和右键菜单只把链接交给后台（Chrome 的 service worker / Firefox 的 event page），任务状态写进 `storage.session`，弹窗订阅它来渲染。后台重启时仍标记为「进行中」的任务一定已经中断，会被标成失败并提示重试。
