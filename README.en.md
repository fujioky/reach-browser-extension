...
| Popup: the current post, this link's access control, recent shares | Options: sign in with the admin account, default share settings |
...
- **The background runs the share; the popup only shows it.** A popup is closed the moment it loses focus, so it cannot hold a request that takes tens of seconds. The popup and the context menu hand the link to the background (Chrome's service worker, Firefox's event page), which writes job state to `storage.session`; the popup subscribes and renders. A job still marked running when the background starts again was necessarily cut off, and is marked failed with a retry button.
...
| `storage` | Sign-in, default settings, recent shares |
...
POST   /api/extension/share     Authorization: Bearer <token>
       ...
       → NDJSON:
         ...
         {"type":"done","ok":true,"shareUrl":"…","adminUrl":"…","title":"…","reused":false,"fetchedAt":"…","warnings":[]}
       or {"type":"done","ok":false,"error":"内容不存在或已删除"}
```

Every endpoint answers 401 for a revoked token, which signs the extension out.
...
utils/
  ...
  jobs.ts             job state, which pages can be shared
  access.ts           access-control presets
